...
if __name__ == '__main__':
    ...
    try:
        ...
        Robotiq.goto(robotiq_client, pos=0.006, speed=config['gripper_speed'], force=config['gripper_force'], block=False)
        rospy.sleep(0.5)

        motion_primitives.set_joint([153.26, -62.00, -138.19, -70.93, 90.95, 153.32]) # place 1
        ...
        servo_pub.publish(140)
        rospy.sleep(2)
        dynamixel.set_position(195)
        rospy.sleep(2)
        dynamixel.set_position(90)
        ...
        # Tilt
        tilt.tilt(center, axis, int(90-theta_0), 0.02)
        rospy.sleep(2)
        ...
        print width
        # Second regrasp_pose
        regrasp.regrasp_test(np.multiply(axis, -1), int(tuck_angle), tcp_speed, width, theta_init=theta_0+psi_regrasp-theta_tilt+1, psi_init=psi_regrasp)
        motion_primitives.set_pose_relative([0,-0.02,0.1])
